...
function mockFileStructure(libraryName: string, libraryVersion: string, modelPrefix: string, dest: string) {
  ...
  fs.writeFileSync(filesModelPrefix + '.diff.x.csv', modelPrefix + '.diff.x.csv')
  fs.writeFileSync(filesModelPrefix + '.diff.x.html', modelPrefix + '.diff.x.html')
  ...
  fs.writeFileSync(path.join(dest, `${libraryName}_${libraryVersion}.html`), '${libraryName}_${libraryVersion}.html')
}
...
describe('collect.ts', () => {
  ...
  it('Copy HTML files', () => {
    ...
    copyHtmlFilesSync(libraryName, libraryVersion, branchOM, omLibTestingDir, targetDir)

    const files = fs.readdirSync(path.join(targetDir, branchOM, `${libraryName}_${libraryVersion}`, 'files'))
    ...
    expect(
      fs.existsSync(path.join(targetDir, branchOM, `${libraryName}_${libraryVersion}`, `${libraryName}_${libraryVersion}.html`))
    ).toBe(true)
